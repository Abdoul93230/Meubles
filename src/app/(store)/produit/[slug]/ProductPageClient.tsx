...
export default function ProductPageClient({ product }: { product: Product }) {
  ...
  const [imgErrors, setImgErrors] = useState<boolean[]>(images.map(() => false));
  ...
  const savings = product.original_price ? product.original_price - product.price : null;
  ...
  return (
    <div className="bg-white min-h-screen pb-[180px] lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
              ...
              <div className="flex lg:hidden flex-row gap-2 mb-3 overflow-x-auto pb-1">
                {images.map((src, i) => (
                  <button key={i} onClick={() => setActiveImg(i)}
                    ...
                    {hasImg(i)
                      ? <Image src={src} alt="" fill className="object-cover" onError={() => setImgErrors(prev => { const n=[...prev]; n[i]=true; return n; })} />
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 flex flex-col gap-3">
                <div className="relative rounded-2xl overflow-hidden bg-gray-50" style={{ aspectRatio: '4/3' }}>
                  {hasImg(activeImg)
                    ? <Image src={images[activeImg]} alt={product.name} fill className="object-cover object-center" priority
                        onError={() => setImgErrors(prev => { const n=[...prev]; n[activeImg]=true; return n; })} />
                    : <div className="absolute inset-0 flex items-center justify-center" style={{ background: FALLBACK_COLORS[0] }}>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-[var(--surface)] rounded-xl border border-[var(--border)]">
              ...
              <Link href="/contact" className="text-xs font-semibold text-[var(--gold)] hover:underline">Voir boutique</Link>
            </div>

            <div>
              <h2 className="text-lg font-bold leading-snug mb-2" style={{ color: 'var(--gold)' }}>{product.name}</h2>
              <p className="text-sm text-gray-500 leading-relaxed">{product.description}</p>
              <div className="flex items-center justify-between mt-2">
                <div className="flex items-center gap-1.5">
                  {[1,2,3,4,5].map((s) => <Star key={s} size={13} className="text-gray-300" fill="#D1D5DB" />)}
            ...
            <div>
              <div className="flex items-baseline gap-3">
                ...
                {product.original_price && <span className="text-base text-gray-400 line-through">{formatPrice(product.original_price)}</span>}
              </div>
              {savings && (
            ...
            <div ref={ctaRef} className="border border-[var(--border)] rounded-2xl p-4 space-y-3">
              ...
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                ...
                {WA_SVG} COMMANDER SUR WHATSAPP
              </a>
              <a href="tel:+22791414165"
                ...
                <Phone size={15} /> APPELER : +227 91 41 41 65
              </a>
            ...
            <button onClick={() => setOfferOpen(true)}
              ...
              <MessageCircle size={15} /> Faire une Offre
            </button>
            ...
            {product.stock_status === 'on_order' && (
              <div className="border border-orange-200 bg-orange-50 rounded-xl p-3">
                ...
                <p className="text-xs text-orange-600 ml-6">Cet article est disponible en pré-commande.</p>
                <p className="text-xs font-semibold text-orange-600 ml-6 mt-1">Livraison estimée : {product.delivery_days} jours</p>
              </div>
            )}
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
          <div className="space-y-0">
            {[
              {
                ...
                content: (
                  <div className="text-sm text-gray-600 leading-relaxed space-y-3 py-4">
                    ...
                    <p>Fabriqué avec des matériaux soigneusement sélectionnés, ce meuble allie esthétique raffinée et durabilité.</p>
            ...
            <div className="border border-[var(--border)] rounded-2xl p-4 mt-6 space-y-0 divide-y divide-[var(--border)]">
              ...
              <Link href="/contact" className="flex items-center justify-between py-3 text-sm text-gray-600 hover:text-[var(--gold)] transition-colors">
                <div className="flex items-center gap-2"><MapPin size={14} className="text-[var(--gold)]" /> Bienvenue dans notre showroom</div>
                <ChevronRight size={14} className="text-gray-300" />
              </Link>
              <Link href="/contact" className="flex items-center justify-between py-3 text-sm text-gray-600 hover:text-[var(--gold)] transition-colors">
                <div className="flex items-center gap-2"><Phone size={14} className="text-[var(--gold)]" /> Demander un rendez-vous design gratuit</div>
                <ChevronRight size={14} className="text-gray-300" />
              </Link>
            </div>
          </div>
          ...
          <div className="hidden lg:block">
            <div className={`sticky top-[100px] border border-[var(--border)] rounded-2xl p-4 space-y-3 transition-all duration-300 ${
              ...
              <div className="flex gap-3 items-start">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-baseline gap-2 mt-1">
                    ...
                    {product.original_price && (
                      <span className="text-[10px] text-[var(--gold)] font-semibold">Économisez {getDiscountPercent(product.price, product.original_price!)}%</span>
                    )}
                  </div>
                </div>
              </div>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: '#25D366' }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.555 4.126 1.526 5.863L0 24l6.335-1.508A11.95 11.95 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 0 1-4.99-1.363l-.358-.213-3.76.895.944-3.658-.234-.376A9.808 9.808 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg>
            ...
            <div className="space-y-3">
              ...
              {related.map((rel, i) => (
                <div key={rel.id} className="flex items-center gap-4 p-4 border-2 rounded-2xl transition-all hover:border-[var(--gold)]" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
                    {rel.images?.[0] ? <Image src={rel.images[0]} alt={rel.name} fill className="object-cover" /> : <div className="absolute inset-0" style={{ background: FALLBACK_COLORS[(i+1) % FALLBACK_COLORS.length] }} />}
                  </div>
                  <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between p-4 bg-[var(--surface)] rounded-2xl border border-[var(--border)]">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400">{1 + related.length} articles</p>
                </div>
                <a href={waUrl} target="_blank" rel="noopener noreferrer"
              ...
              return (
                <div key={label} className="group border border-[var(--border)] rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
                  <div className="relative aspect-square bg-gray-50">
                    {p.images?.[0] ? <Image src={p.images[0]} alt={p.name} fill className="object-cover" /> : <div className="absolute inset-0" style={{ background: FALLBACK_COLORS[idx % FALLBACK_COLORS.length] }} />}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {youMayLike.map((p, i) => (
              <Link key={p.id} href={`/produit/${p.slug}`} className="group border border-[var(--border)] rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative aspect-square bg-gray-50">
                  {p.images?.[0] ? <Image src={p.images[0]} alt={p.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" /> : <div className="absolute inset-0" style={{ background: FALLBACK_COLORS[i % FALLBACK_COLORS.length] }} />}
                </div>
                <div className="p-3">
                  <p className="text-xs font-medium text-[var(--dark)] line-clamp-2 mb-1 leading-snug">{p.name}</p>
      ...
      {/* ── Mobile sticky CTA ── */}
      <div className={`lg:hidden fixed bottom-[60px] left-0 right-0 z-40 bg-white border-t border-[var(--border)] px-4 py-2.5 transition-all duration-300 ${
        ...
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-sm font-bold text-[var(--dark)]">{formatPrice(product.price)}</span>
          {savings && <span className="text-xs text-[var(--gold)]">Économisez {getDiscountPercent(product.price, product.original_price!)}%</span>}
        </div>
        <div className="flex gap-2">
          <a href={waUrl} target="_blank" rel="noopener noreferrer"
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border-2" style={{ borderColor: 'var(--gold)' }}>
              <div className="flex items-start justify-between p-5 pb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 leading-relaxed">Soumettez votre meilleur prix pour {product.name}</p>
                </div>
                <button onClick={() => setOfferOpen(false)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400"><X size={18} /></button>
              </div>
              <div className="px-5 pb-5 space-y-4">
                ...
                <div className="bg-[var(--gold-pale)] border border-[var(--gold)] rounded-xl px-3 py-2">
                  <p className="text-[11px] text-[var(--gold)] font-medium">💡 Les offres dans les 20% du prix ont plus de chances d'être acceptées</p>
                </div>
                <div>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setOfferOpen(false)} className="flex-1 py-3 rounded-xl border border-[var(--border)] text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors">Annuler</button>
                  <button className="flex-1 py-3 rounded-xl font-bold text-sm transition-all hover:opacity-90"
                    style={{ background: 'var(--gold)', color: 'white', opacity: offerPrice ? 1 : 0.5 }} disabled={!offerPrice}>
                    Soumettre l'Offre
                  </button>
                </div>
                <p className="text-[10px] text-blue-500 leading-relaxed">ℹ️ Votre offre sera examinée par notre équipe.</p>
              </div>
            </div>
          </div>
        </>
